...
| Product name and presentation | Own logo and `Chat Cleanup — Bulk Archive & Delete` title; ChatGPT appears only in descriptive compatibility text and unmodified host-page context; explicit independent/not-endorsed statement | **Reduced trademark review risk.** Chat Cleanup remains the primary brand and the title no longer contains an OpenAI mark | Keep OpenAI marks out of the product name, icon, and promotional tile; keep compatibility references factual and the independent-product disclosure visible |
